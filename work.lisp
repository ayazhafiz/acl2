...
;; Module "Hypothesis Collection" attempts to collect and introduce reasonable
;; hypotheses to a conjecture, namely by collecting the guards present on
;; definitions of function names used in the conjecture.
...
;; Module "Hypothesis Extraction" retrieves hypotheses associated with left-hand
;; terms from theorems in a particular world.
;; The idea is that common hypotheses reused across many theorems are likely to
;; be "good" or "useful" in some sense, and can be introduced as hypotheses for
;; conjectures which fail to be proven with the standard hypotheses introduced
;; to a conjecture (namely those collected from function guards; see "Hypothesis
;; Collection").

(defconst *extract-hyps--NORMV-PREFIX* "__THM_GEN__PMHOLE__")

;; Generates n variable names of the form extract-hyps--NORMV-PREFIX.
;; Variables are generated in "reverse order", in that the nth variable is first
;; and the 1st variable is last.
;; We don't check for collisions here, but that is probably a reasonable
;; assumption to make.
(define extract-hyps--gen-n-vars (n)
  :mode :program
  (if (zerop n) nil
      (cons (genvar nil *extract-hyps--NORMV-PREFIX* n '()) (extract-hyps--gen-n-vars (- n 1)))))

;; Given some term and hypothesis, replaces the free variables in that term
;; (and hypothesis) with variables generated by `extract-hyps--gen-n-vars`. This
;; enables us to compare terms with the same free variable names later on.
;;
;; If the set of free variables in the hyps is not a subset of those in the
;; term, unbound-hyps-v-p is T.
(define extract-hyps--normalize-term-var-names (term hyps)
  :mode :program
  (b* ((termvars (all-vars term))
       (hypsvars (all-vars hyps))
       (unbound-hyps-v-p (not (subsetp hypsvars termvars)))
       (freshvars (extract-hyps--gen-n-vars (len termvars)))
       ; `all-vars` returns vars in reverse order of presence in the term, but
       ; `freshvars` returns terms generated in reverse order as well so direct
       ; substitution preserves the original ordering.
       (freshterm (fsubcor-var termvars freshvars term))
       (freshhyps (fsubcor-var termvars freshvars hyps)))
      (mv unbound-hyps-v-p freshterm freshhyps freshvars)))

(define extract-hyps-maps--get-all-lemma-info (fns wrld state)
  :mode :program :stobjs state
  (if (endp fns) nil
      (b* ((ens (ens-maybe-brr state))
           (fn (car fns)))
          (append
           ; Pulled from pl-fn (<root>/other-events.lisp)
           (info-for-lemmas (getpropc fn 'lemmas nil wrld) t (ens-maybe-brr state) wrld)
           (info-for-linear-lemmas (getpropc fn 'linear-lemmas nil wrld) t ens wrld)
           (info-for-type-prescriptions (getpropc fn 'type-prescriptions nil wrld) t ens wrld)
           (info-for-forward-chaining-rules (getpropc fn 'forward-chaining-rules nil wrld) t ens wrld)
           (let ((elim-rule (getpropc fn 'eliminate-destructors-rule nil wrld)))
                (and elim-rule (info-for-eliminate-destructors-rule elim-rule t ens wrld)))
           (info-for-induction-rules (getpropc fn 'induction-rules nil wrld) t ens wrld)
           (extract-hyps-maps--get-all-lemma-info (cdr fns) wrld state)))))

;; Generates a hyps-map of form
;;   term -> hyp -> (free-vars , hits)
;; From a set of lemma infos, retrieved from the world.
(define extract-hyps-maps--from-lemma-info (lemma-infos)
  :mode :program
  (if (endp lemma-infos) nil
      (b* ((lemma (car lemma-infos))
           (rst (extract-hyps-maps--from-lemma-info (cdr lemma-infos)))
           ((mv unbound-hyps-v-p term hyp free-vars) (extract-hyps--normalize-term-var-names
                                                      (cadr (assoc :lhs lemma))
                                                      (cadr (assoc :hyps lemma)))))
          (cond
           ; The hypothesis is vacuous. This is totally useless for us; move on.
           ((equal hyp T) rst)
           ; The hypothesis contains a variable that is not used in the actual
           ; term. It's probably not worth introducing this on our side, since it
           ; could cause a collision that is (somewhat) tricky to avoid, and is
           ; unlikely to be considered a "useful" theorem anyway.
           (unbound-hyps-v-p rst)
           ; 1. this term and the present hypothesis already exist.
           ;    Increase the number of hits and move on.
           ((and (assoc term rst) (assoc hyp (aget term rst)))
            (b* ((term-alist (aget term rst))
                 (hyp-alist (aget hyp term-alist))
                 (old-hits (aget 'hits hyp-alist))
                 (new-hyps-alist (put-assoc 'hits (1+ old-hits) hyp-alist))
                 (new-term-alist (put-assoc hyp new-hyps-alist term-alist)))
                (put-assoc term new-term-alist rst)))
           ; 2. this term exists, but the present hypothesis doesn't.
           ;    Add it.
           ((assoc term rst)
            (b* ((term-alist (aget term rst))
                 (hyp-alist (acons 'hits 1 (acons 'free-vars free-vars nil)))
                 (new-term-alist (put-assoc hyp hyp-alist term-alist)))
                (put-assoc term new-term-alist rst)))
           ; 3. The term doesn't exist. Add it and a hypothesis.
           (t
            (b* ((hyp-alist (acons 'hits 1 (acons 'free-vars free-vars nil)))
                 (term-alist (acons hyp hyp-alist nil)))
                (put-assoc term term-alist rst)))))))

;; Extracts hypotheses from existing lemmas in the theory.
;; Generates a mapping of
;;   term -> hyp -> (free-vars , hits)
;; where `term` is an LHS term of a lemma with the relevant `hyps`, and `hits`
;; is the number of times this hypothesis has been seen across all lemmas in the
;; theory (up to variable renaming). The idea is that more common hyps are more
;; likely to be "good" or "important", and can be attempted to be introduced as
;; part of generated conjectures whose preliminary hypotheses fail to satisfy
;; anything.
(define extract-hyps-maps-from-theory (st state)
  :mode :program
  :stobjs state
  (b* ((fns (st-all-fns st))
       (fns-lemma-info (extract-hyps-maps--get-all-lemma-info fns (w state) state))
       (hyps-map (extract-hyps-maps--from-lemma-info fns-lemma-info)))
      hyps-map))

(define extract-hyps--rw-hyps (hyps-lst oldvars newvars)
  :mode :program
  (if (endp hyps-lst) nil
      (cons
       (fsubcor-var oldvars newvars (caar hyps-lst))
       (extract-hyps--rw-hyps (cdr hyps-lst) oldvars newvars))))

(define extract-hyps--matching-subcalls (subcalls hyps-map)
  :mode :program
  (if (endp subcalls) nil
      (b* ((rst (extract-hyps--matching-subcalls (cdr subcalls) hyps-map))
           (call (car subcalls))
           (callvars (all-vars call))
           ((mv ?e normcall ?h normvars) (extract-hyps--normalize-term-var-names call 'nil)))
          (append
           (if (assoc-equal normcall hyps-map)
               (extract-hyps--rw-hyps (cdr (assoc-equal normcall hyps-map)) normvars callvars)
               nil)
           rst))))

;; Given a term and a hyps-map, attempts to pattern-match terms in the hyps-map
;; to subterms in `term`. For all such matched patterns, the hyps from the
;; hyps-map associated to the subterm are rewritten to fit the term being
;; matched against.
;; The end result is a list of hypotheses that "fit" on the present term given
;; their usage on other similar-looking terms.
(define extract-hyps-match-on-term ((term pseudo-termp) hyps-map)
  :mode :program
  (b* ((subcalls (all-calls (all-fnnames term) term nil nil)))
      (extract-hyps--matching-subcalls subcalls hyps-map)))
...
(defmacro st** (fns left-size)
  `(st-new (fn-datas ,fns state) ,left-size))
